...
@Component
public class MonthlyClientCreationCountExcelUseCase {
    ...
    public byte[] execute() {
        try (final var workbook = new HSSFWorkbook()){
            final var content = fetchMonthlyClientCreationCountUseCase.execute();
            final var sheet = workbook.createSheet("Monthly Client Creation Count");
            ...
            throw new ApplicationException("Monthly Client Report Error", List.of(error.getMessage()));
        }
    }
