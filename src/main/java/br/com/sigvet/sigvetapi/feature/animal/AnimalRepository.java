...
public interface AnimalRepository extends JpaRepository<AnimalEntity, Long>, JpaSpecificationExecutor<AnimalEntity> {
    ...
    @Query(value = "SELECT * FROM animals a WHERE LOWER(unaccent(a.name)) LIKE unaccent(CONCAT('%', LOWER(?1), '%')) AND u.deleted = false", nativeQuery = true)
    List<AnimalEntity> searchByName(String name);

    List<AnimalEntity> findByClientId(Long id);
}
